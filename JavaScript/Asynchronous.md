...
4. Event Loop
  - 작업이 들어오길 기다렸다가 작업이 들어오면 이를 처리하고, 처리할 작업이 없는 경우에는 잠드는, 끊임없이 돌아가는 자바스크립트 내 루프
  - Call Stack과 Task Queue를 지속적으로 모니터링
  - Call Stack이 비어있는지 확인 후 비어있다면 Task Queue에서 대기 중인 오래도니 작업을 Call Stack으로 Push
...
### 기존 기술과의 차이 - 기존 방식
1. 클라이언트에서 form을 채우고 이를 서버로 제출
2. 서버는 요청 내용에 따라 데이터 처리 후 새로운 웹페이를 작성하여 응답으로 전달  
...
-> 서버에서 모두 처리되던 데이터 처리의 일부분이 이제는 클아이언트 쪽에서 처리되므로 교환되는 데이터량과 처리량이 줄어듦
...
### Axios
자바스크립트에서 사용되는 Promise 기반 HTTP 클라이언트 라이브러리
